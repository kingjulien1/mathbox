import { useContext, FunctionComponent, Dispatch, SetStateAction, useState } from 'react'
...
const ToolItem: FunctionComponent<ToolItemProps> = ({ Icon, setTool, setOptions, active, Menu, tool }) => {
  const [isOpen, setIsOpen] = useState(false)
  return (
    <div>
      <Menu {...{ isOpen, setIsOpen, setOptions }}></Menu>
      <Icon
  ...
  return (
    <nav className="ml-4 px-2 my-2 mx-2 flex flex-col items-center text-gray-900 sm:justify-between absolute top-0 left-0 z-10 rounded-lg bg-gray-100 shadow-lg">
      {tools.map((toolItem, index) => (
        <ToolItem key={index} {...{ ...toolItem, setTool, setOptions }}></ToolItem>
      ))}
    </nav>
  )
}
